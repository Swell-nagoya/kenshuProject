...
<body>
	...
	String actionType = val.equals("削除する") ? "deleteEnter"
			: val.equals("修正する") ? "updateEnter" : val.equals("登録する") ? "insEnter" : "unknown";
	String actionBtn = val.equals("メール送信") ? "go_mail" : "go_submit";
	String header = val.equals("削除する") ? "部屋削除"
			: val.equals("修正する") ? "部屋名修正確認" : val.equals("登録する") ? "新規部屋登録確認" : "unknown";
	%>
	<div class="container">
		<div class="new-btn">
		...
		<form method="post" id="main_form" action="" class="main__form">
			...
			<input type="hidden" name="form_name" id="form_name"
				value="RoomDetail_2" /> <input type="hidden" name="action_cmd"
				id="action_cmd" value="" /> <input type="hidden" name="room_name"
				id="room_name" value="<%=webBean.txt("room_name")%>" /> <input
				...
				name="main_key" id="main_key" value="<%=webBean.txt("main_key")%>" />
			<input type="hidden" name="before_name" id="before_name"
				value="<%=webBean.txt("before_name")%>" />

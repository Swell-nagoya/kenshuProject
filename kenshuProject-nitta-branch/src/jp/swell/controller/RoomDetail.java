...
public class RoomDetail extends ControllerBase {
    ...
    public void doActionProcess() throws AtareSysException {
        ...
        try {
            ...
            // デバッグ用ログ
            System.out.println(
                    "DEBUG: form=" + formName + " act=" + actionCmd + " req=" + requestCmd + " key=" + mainKey);
            ...
            if ("RoomDetail".equals(formName)) {
                ...
                if ("go_next".equals(actionCmd)) {
                    ...
                    } else if ("deletef".equals(requestCmd)) { // ★一覧からの削除は deletef
                        if (!setDb2Web()) {
                            bean.setError("データの取得に失敗しました");
                            forward("RoomList.jsp");
                        } else {
            ...
            } else if ("RoomDetail_2".equals(formName)) { // 確認画面（モード）からの処理
                if ("go_next".equals(actionCmd)) {
                    // JSPから送られてくるコマンド名に合わせて修正
                    ...
                    if ("deleteEnter".equals(requestCmd)) {
                        redirect("RoomList.do");
                    } else {
                        bean.setValue("mode", ""); // 入力モードに戻す
                        forward("RoomDetail.jsp");
            ...
            e.printStackTrace(); // コンソールにエラー詳細を表示
            bean.setError("処理中にエラーが発生しました: " + e.getMessage());
            forward("RoomList.jsp"); // ErrorPageがないため一覧へ戻す
        }
    }
    ...
    public void dbEdit() throws AtareSysException {
        ...
        // IDを確実に取得
        String mainKey = bean.value("room_id");
        if (mainKey == null || mainKey.isEmpty()) {
            mainKey = bean.value("main_key");
        }
    ...
    private boolean inputCheck(RoomDao pRoomDao) throws AtareSysException {
        ...
        if (roomName.length() == 0) {
            errors.put("room_name", "部屋名を入力してください。");
        }
    ...
    private void setInputInfo2Dao2Web() throws AtareSysException {
        WebBean bean = getWebBean();
        if (bean.value("input_info") != null && !bean.value("input_info").isEmpty()) {

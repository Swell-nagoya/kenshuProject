...
public class FileDao implements Serializable {
	...
	public void setFileDaoForJoin(HashMap<String, String> map, FileDao dao) throws AtareSysException {
		...
		dao.setFileId(DbI.chara(nullToEmpty(getValue(map, "files___file_id", "file_id"))));
		dao.setUserInfoId(DbI.chara(nullToEmpty(getValue(map, "files___user_info_id", "user_info_id"))));
		dao.setFileName(DbI.chara(nullToEmpty(getValue(map, "files___file_name", "file_name"))));
		dao.setFilePath(DbI.chara(nullToEmpty(getValue(map, "files___file_path", "file_path"))));
		dao.setUploadDate(DbI.chara(nullToEmpty(getValue(map, "files___upload_date", "upload_date"))));
		dao.setFileKey(DbI.chara(nullToEmpty(getValue(map, "files___file_key", "file_key"))));
		dao.setMimeType(DbI.chara(nullToEmpty(getValue(map, "files___mime_type", "mime_type"))));
		dao.setSystemFileName(DbI.chara(nullToEmpty(getValue(map, "files___system_file_name", "system_file_name"))));
		dao.setUploadUserId(DbI.chara(nullToEmpty(getValue(map, "files___upload_user_id", "upload_user_id"))));
		dao.setExpirationDate(DbI.chara(nullToEmpty(getValue(map, "files___expiration_date", "expiration_date"))));

		dao.setFirstName(nullToEmpty(getValue(map, "send_user_first_name", "first_name")));
		dao.setLastName(nullToEmpty(getValue(map, "send_user_last_name", "last_name")));

		// アップロードユーザー名は別SQLで取ることにしました。
		dao.setUploaderFirstName("");
		dao.setUploaderLastName("");

	}

	private void setUploaderNameByUserId(String uploadUserId) throws AtareSysException {
		...
		String sql = "SELECT "
				...
				+ "last_name "
				+ "FROM user_info "
				+ "WHERE user_info_id = " + DbS.chara(uploadUserId);
		...
		HashMap<String, String> map = rs.get(0);
		setUploaderFirstName(nullToEmpty(map.get("first_name")));
		setUploaderLastName(nullToEmpty(map.get("last_name")));
	}

	private String getValue(HashMap<String, String> map, String key1, String key2) {
	...
	public static ArrayList<FileDao> dbSelectList(FileDao myclass, LinkedHashMap<String, String> sortKey,
		...
		String sql = "SELECT "
				...
				+ "send_user.first_name AS send_user_first_name, "
				+ "send_user.last_name AS send_user_last_name "
				+ "FROM files "
				+ "JOIN user_info AS send_user ON files.user_info_id = send_user.user_info_id "
		...
		for (HashMap<String, String> map : rs) {
			...
			dao.setUploaderNameByUserId(dao.getUploadUserId());

...
public class FileDao implements Serializable {
	...
	public String getsendUserName() {
		return sendLastName + " " + sendFirstName;
	}
	...
	void setsendLastName(String sendLastName) {
	...
	public void setFileDao(HashMap<String, String> map, FileDao dao) throws AtareSysException {
		...
		dao.setUploaderLastName(DbI.chara(map.get("up_last")));
		dao.setUploaderFirstName(DbI.chara(map.get("up_first")));
		dao.setsendLastName(DbI.chara(map.get("send_last")));
		dao.setsendFirstName(DbI.chara(map.get("send_first")));
	}
	...
	public ArrayList<FileDao> getAllFiles() throws AtareSysException {
		String sql = "SELECT "
				...
				+ " expiration_date  "
				+ " FROM files  "
				+ " join user_info on files.user_info_id = user_info.user_info_id"; //user_infoとfilesのjoin句
		...
		for (HashMap<String, String> map : rs) {
			...
			dao.setFileId(map.get("file_id"));//UserInfoId->FileId
	...
	public ArrayList<FileDao> getSendFiles() throws AtareSysException {
		String sql = "SELECT "
				...
				+ " upload_user_id "
				+ " from files "
				+ " join  user_info on files.upload_user_id = user_info.user_info_id";
		List<HashMap<String, String>> rs = DbBase.dbSelect(sql);
		ArrayList<FileDao> files = new ArrayList<>();
		for (HashMap<String, String> map : rs) {
			FileDao dao = new FileDao();
			...
			dao.setUserInfoId(map.get("user_info_id"));
			dao.setsendLastName(map.get("last_name"));
	...
	public FileDao() {
		...
		fieldsArray.put("send_last", "send_info.last_name");
		fieldsArray.put("send_first", "send_info.first_name");
		fieldsArray.put("up_last", "up_info.last_name");
		fieldsArray.put("up_first", "up_info.first_name");

...
public class UserLogin extends ControllerBase {
	...
	public void doActionProcess() throws AtareSysException {
		...
		//実施内容①画面に入力されたテキストデータを取得できているのか
		//入力されたテキストデータを取得できていることを確認。
		System.out.println("入力されたテキストデータを取得できているか確認");
        System.out.println("Username の入力データ "+ bean.value("ac"));         //Username の入力データ
        System.out.println("Password の入力データ " + bean.value("ko") + "\n");         //Password の入力データ
	...
	private boolean inputCheck() throws AtareSysException {
		...
		if (!userLoginInfo.login(bean.value("ac"), bean.value("ko"))) {
			...
			//実施内容③ 間違っていた場合のエラー文は表示されているのか
			//下記より間違っていた場合にエラー文が表示されることを確認。
			System.out.println("usernameかpasswordが違います");

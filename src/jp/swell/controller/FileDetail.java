...
public class FileDetail extends ControllerBase {
	...
	private ArrayList<FileDao> FileUpload(HttpServletRequest request, String pUserInfoId)
		...
		String filePath = "C:/workspace/kenshuProject/WebContent/upload"; //保存先フォルダのパス設定
	...
	public boolean dbDelete(String pFileId) throws AtareSysException {
		// filesテーブルからレコードを削除
		String sqlDeleteuser = "DELETE FROM user_files WHERE file_id = " + DbS.chara(pFileId);
		DbBase.dbExec(sqlDeleteuser);
		String sqlDeleteReserve = "DELETE FROM reserve_file WHERE file_id = " + DbS.chara(pFileId);
		DbBase.dbExec(sqlDeleteReserve);

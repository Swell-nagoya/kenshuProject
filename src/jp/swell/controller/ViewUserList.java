...
public class ViewUserList extends ControllerBase {
	...
	/**
	 * jp.patasys.cloudbiz.common.ControllerBase のメソッドをオーバライドする。 ここで、コントローラの処理を記述する.
	 *
	...
	public void doActionProcess() throws AtareSysException {
		...
		if (loginInfo != null) {
			bean.setValue("login_admin", loginInfo.getAdmin());
		}
		if ("ViewUserList".equals(bean.value("form_name"))) {
			...
			if ("search".equals(bean.value("action_cmd"))) {
				...
				formClear();
				searchList();
			} else if ("bulk_enable".equals(bean.value("action_cmd"))) {
				String csv = bean.value("selected_user_ids_csv");
				...
				searchList();
			} else if ("bulk_disable".equals(bean.value("action_cmd"))) {
				String csv = bean.value("selected_user_ids_csv");
	...
	private void searchList() throws AtareSysException {
		...
		String listSearchState = bean.value("list_search_state");
		dao.setSearchState(listSearchState);
